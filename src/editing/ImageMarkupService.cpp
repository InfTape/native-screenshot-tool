...
bool ImageMarkupService::DrawArrow(capture::CapturedImage& image,
    ...
    HBITMAP bitmap =
        CreateDIBSection(memory_dc, &bitmap_info, DIB_RGB_COLORS, &bitmap_bits, nullptr, 0);
    if (bitmap == nullptr || bitmap_bits == nullptr) {
        if (bitmap != nullptr) {
            DeleteObject(bitmap);
        }
        DeleteDC(memory_dc);
        error_message = L"创建箭头绘制位图失败。";
        return false;
    }
    ...
    const bool draw_succeeded = common::DrawArrowOnHdc(memory_dc,
                                                       full_image_rect,
                                                       &clamped_clip,
                                                       clamped_start,
                                                       clamped_end,
    ...
    if (!draw_succeeded) {
        error_message = d2d_error_message.empty() ? L"Direct2D 绘制箭头失败。" : d2d_error_message;
        return false;
    }
